/*
Okx Rest API

OpenAPI specification for Okx cryptocurrency exchange - Rest API

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package rest

import (
	"encoding/json"
)

// checks if the GetAffiliateInviteeDetailV5RespData type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetAffiliateInviteeDetailV5RespData{}

// GetAffiliateInviteeDetailV5RespData struct for GetAffiliateInviteeDetailV5RespData
type GetAffiliateInviteeDetailV5RespData struct {
	// Accumulated Amount of trading fee in USDT  If there is no any fee, 0 will be returned
	AccFee *string `json:"accFee,omitempty"`
	// Affiliate invite code that the invitee registered/recalled via
	AffiliateCode *string `json:"affiliateCode,omitempty"`
	// Accumulated amount of deposit in USDT  If user has not deposited, 0 will be returned
	DepAmt *string `json:"depAmt,omitempty"`
	// Timestamp that the first trade is completed after the latest rebate relationship is established with the parent affiliate   Unix timestamp in millisecond format, e.g. 1597026383085  If user has not traded, \"\" will be returned
	FirstTradeTime *string `json:"firstTradeTime,omitempty"`
	// Invitee's relative level to the affiliate  If the user is a invitee, the level will be `2`.
	InviteeLevel *string `json:"inviteeLevel,omitempty"`
	// Self rebate rate of the invitee (in decimal), e.g. `0.01` represents `10%`
	InviteeRebateRate *string `json:"inviteeRebateRate,omitempty"`
	// Timestamp that the rebate relationship is established, Unix timestamp in millisecond format, e.g. `1597026383085`
	JoinTime *string `json:"joinTime,omitempty"`
	// KYC2 verification time. Unix timestamp in millisecond format and the precision is in day  If user has not passed KYC2, \"\" will be returned
	KycTime *string `json:"kycTime,omitempty"`
	// Invitee trading fee level, e.g. Lv1
	Level *string `json:"level,omitempty"`
	// User country or region. e.g. \"United Kingdom\"
	Region *string `json:"region,omitempty"`
	// Total commission earned from the invitee, unit in `USDT`
	TotalCommission *string `json:"totalCommission,omitempty"`
	// Accumulated Trading volume in the current month in USDT  If user has not traded, 0 will be returned
	VolMonth *string `json:"volMonth,omitempty"`
}

// NewGetAffiliateInviteeDetailV5RespData instantiates a new GetAffiliateInviteeDetailV5RespData object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetAffiliateInviteeDetailV5RespData() *GetAffiliateInviteeDetailV5RespData {
	this := GetAffiliateInviteeDetailV5RespData{}
	var accFee string = ""
	this.AccFee = &accFee
	var affiliateCode string = ""
	this.AffiliateCode = &affiliateCode
	var depAmt string = ""
	this.DepAmt = &depAmt
	var firstTradeTime string = ""
	this.FirstTradeTime = &firstTradeTime
	var inviteeLevel string = ""
	this.InviteeLevel = &inviteeLevel
	var inviteeRebateRate string = ""
	this.InviteeRebateRate = &inviteeRebateRate
	var joinTime string = ""
	this.JoinTime = &joinTime
	var kycTime string = ""
	this.KycTime = &kycTime
	var level string = ""
	this.Level = &level
	var region string = ""
	this.Region = &region
	var totalCommission string = ""
	this.TotalCommission = &totalCommission
	var volMonth string = ""
	this.VolMonth = &volMonth
	return &this
}

// NewGetAffiliateInviteeDetailV5RespDataWithDefaults instantiates a new GetAffiliateInviteeDetailV5RespData object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetAffiliateInviteeDetailV5RespDataWithDefaults() *GetAffiliateInviteeDetailV5RespData {
	this := GetAffiliateInviteeDetailV5RespData{}
	var accFee string = ""
	this.AccFee = &accFee
	var affiliateCode string = ""
	this.AffiliateCode = &affiliateCode
	var depAmt string = ""
	this.DepAmt = &depAmt
	var firstTradeTime string = ""
	this.FirstTradeTime = &firstTradeTime
	var inviteeLevel string = ""
	this.InviteeLevel = &inviteeLevel
	var inviteeRebateRate string = ""
	this.InviteeRebateRate = &inviteeRebateRate
	var joinTime string = ""
	this.JoinTime = &joinTime
	var kycTime string = ""
	this.KycTime = &kycTime
	var level string = ""
	this.Level = &level
	var region string = ""
	this.Region = &region
	var totalCommission string = ""
	this.TotalCommission = &totalCommission
	var volMonth string = ""
	this.VolMonth = &volMonth
	return &this
}

// GetAccFee returns the AccFee field value if set, zero value otherwise.
func (o *GetAffiliateInviteeDetailV5RespData) GetAccFee() string {
	if o == nil || IsNil(o.AccFee) {
		var ret string
		return ret
	}
	return *o.AccFee
}

// GetAccFeeOk returns a tuple with the AccFee field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAffiliateInviteeDetailV5RespData) GetAccFeeOk() (*string, bool) {
	if o == nil || IsNil(o.AccFee) {
		return nil, false
	}
	return o.AccFee, true
}

// HasAccFee returns a boolean if a field has been set.
func (o *GetAffiliateInviteeDetailV5RespData) HasAccFee() bool {
	if o != nil && !IsNil(o.AccFee) {
		return true
	}

	return false
}

// SetAccFee gets a reference to the given string and assigns it to the AccFee field.
func (o *GetAffiliateInviteeDetailV5RespData) SetAccFee(v string) {
	o.AccFee = &v
}

// GetAffiliateCode returns the AffiliateCode field value if set, zero value otherwise.
func (o *GetAffiliateInviteeDetailV5RespData) GetAffiliateCode() string {
	if o == nil || IsNil(o.AffiliateCode) {
		var ret string
		return ret
	}
	return *o.AffiliateCode
}

// GetAffiliateCodeOk returns a tuple with the AffiliateCode field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAffiliateInviteeDetailV5RespData) GetAffiliateCodeOk() (*string, bool) {
	if o == nil || IsNil(o.AffiliateCode) {
		return nil, false
	}
	return o.AffiliateCode, true
}

// HasAffiliateCode returns a boolean if a field has been set.
func (o *GetAffiliateInviteeDetailV5RespData) HasAffiliateCode() bool {
	if o != nil && !IsNil(o.AffiliateCode) {
		return true
	}

	return false
}

// SetAffiliateCode gets a reference to the given string and assigns it to the AffiliateCode field.
func (o *GetAffiliateInviteeDetailV5RespData) SetAffiliateCode(v string) {
	o.AffiliateCode = &v
}

// GetDepAmt returns the DepAmt field value if set, zero value otherwise.
func (o *GetAffiliateInviteeDetailV5RespData) GetDepAmt() string {
	if o == nil || IsNil(o.DepAmt) {
		var ret string
		return ret
	}
	return *o.DepAmt
}

// GetDepAmtOk returns a tuple with the DepAmt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAffiliateInviteeDetailV5RespData) GetDepAmtOk() (*string, bool) {
	if o == nil || IsNil(o.DepAmt) {
		return nil, false
	}
	return o.DepAmt, true
}

// HasDepAmt returns a boolean if a field has been set.
func (o *GetAffiliateInviteeDetailV5RespData) HasDepAmt() bool {
	if o != nil && !IsNil(o.DepAmt) {
		return true
	}

	return false
}

// SetDepAmt gets a reference to the given string and assigns it to the DepAmt field.
func (o *GetAffiliateInviteeDetailV5RespData) SetDepAmt(v string) {
	o.DepAmt = &v
}

// GetFirstTradeTime returns the FirstTradeTime field value if set, zero value otherwise.
func (o *GetAffiliateInviteeDetailV5RespData) GetFirstTradeTime() string {
	if o == nil || IsNil(o.FirstTradeTime) {
		var ret string
		return ret
	}
	return *o.FirstTradeTime
}

// GetFirstTradeTimeOk returns a tuple with the FirstTradeTime field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAffiliateInviteeDetailV5RespData) GetFirstTradeTimeOk() (*string, bool) {
	if o == nil || IsNil(o.FirstTradeTime) {
		return nil, false
	}
	return o.FirstTradeTime, true
}

// HasFirstTradeTime returns a boolean if a field has been set.
func (o *GetAffiliateInviteeDetailV5RespData) HasFirstTradeTime() bool {
	if o != nil && !IsNil(o.FirstTradeTime) {
		return true
	}

	return false
}

// SetFirstTradeTime gets a reference to the given string and assigns it to the FirstTradeTime field.
func (o *GetAffiliateInviteeDetailV5RespData) SetFirstTradeTime(v string) {
	o.FirstTradeTime = &v
}

// GetInviteeLevel returns the InviteeLevel field value if set, zero value otherwise.
func (o *GetAffiliateInviteeDetailV5RespData) GetInviteeLevel() string {
	if o == nil || IsNil(o.InviteeLevel) {
		var ret string
		return ret
	}
	return *o.InviteeLevel
}

// GetInviteeLevelOk returns a tuple with the InviteeLevel field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAffiliateInviteeDetailV5RespData) GetInviteeLevelOk() (*string, bool) {
	if o == nil || IsNil(o.InviteeLevel) {
		return nil, false
	}
	return o.InviteeLevel, true
}

// HasInviteeLevel returns a boolean if a field has been set.
func (o *GetAffiliateInviteeDetailV5RespData) HasInviteeLevel() bool {
	if o != nil && !IsNil(o.InviteeLevel) {
		return true
	}

	return false
}

// SetInviteeLevel gets a reference to the given string and assigns it to the InviteeLevel field.
func (o *GetAffiliateInviteeDetailV5RespData) SetInviteeLevel(v string) {
	o.InviteeLevel = &v
}

// GetInviteeRebateRate returns the InviteeRebateRate field value if set, zero value otherwise.
func (o *GetAffiliateInviteeDetailV5RespData) GetInviteeRebateRate() string {
	if o == nil || IsNil(o.InviteeRebateRate) {
		var ret string
		return ret
	}
	return *o.InviteeRebateRate
}

// GetInviteeRebateRateOk returns a tuple with the InviteeRebateRate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAffiliateInviteeDetailV5RespData) GetInviteeRebateRateOk() (*string, bool) {
	if o == nil || IsNil(o.InviteeRebateRate) {
		return nil, false
	}
	return o.InviteeRebateRate, true
}

// HasInviteeRebateRate returns a boolean if a field has been set.
func (o *GetAffiliateInviteeDetailV5RespData) HasInviteeRebateRate() bool {
	if o != nil && !IsNil(o.InviteeRebateRate) {
		return true
	}

	return false
}

// SetInviteeRebateRate gets a reference to the given string and assigns it to the InviteeRebateRate field.
func (o *GetAffiliateInviteeDetailV5RespData) SetInviteeRebateRate(v string) {
	o.InviteeRebateRate = &v
}

// GetJoinTime returns the JoinTime field value if set, zero value otherwise.
func (o *GetAffiliateInviteeDetailV5RespData) GetJoinTime() string {
	if o == nil || IsNil(o.JoinTime) {
		var ret string
		return ret
	}
	return *o.JoinTime
}

// GetJoinTimeOk returns a tuple with the JoinTime field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAffiliateInviteeDetailV5RespData) GetJoinTimeOk() (*string, bool) {
	if o == nil || IsNil(o.JoinTime) {
		return nil, false
	}
	return o.JoinTime, true
}

// HasJoinTime returns a boolean if a field has been set.
func (o *GetAffiliateInviteeDetailV5RespData) HasJoinTime() bool {
	if o != nil && !IsNil(o.JoinTime) {
		return true
	}

	return false
}

// SetJoinTime gets a reference to the given string and assigns it to the JoinTime field.
func (o *GetAffiliateInviteeDetailV5RespData) SetJoinTime(v string) {
	o.JoinTime = &v
}

// GetKycTime returns the KycTime field value if set, zero value otherwise.
func (o *GetAffiliateInviteeDetailV5RespData) GetKycTime() string {
	if o == nil || IsNil(o.KycTime) {
		var ret string
		return ret
	}
	return *o.KycTime
}

// GetKycTimeOk returns a tuple with the KycTime field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAffiliateInviteeDetailV5RespData) GetKycTimeOk() (*string, bool) {
	if o == nil || IsNil(o.KycTime) {
		return nil, false
	}
	return o.KycTime, true
}

// HasKycTime returns a boolean if a field has been set.
func (o *GetAffiliateInviteeDetailV5RespData) HasKycTime() bool {
	if o != nil && !IsNil(o.KycTime) {
		return true
	}

	return false
}

// SetKycTime gets a reference to the given string and assigns it to the KycTime field.
func (o *GetAffiliateInviteeDetailV5RespData) SetKycTime(v string) {
	o.KycTime = &v
}

// GetLevel returns the Level field value if set, zero value otherwise.
func (o *GetAffiliateInviteeDetailV5RespData) GetLevel() string {
	if o == nil || IsNil(o.Level) {
		var ret string
		return ret
	}
	return *o.Level
}

// GetLevelOk returns a tuple with the Level field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAffiliateInviteeDetailV5RespData) GetLevelOk() (*string, bool) {
	if o == nil || IsNil(o.Level) {
		return nil, false
	}
	return o.Level, true
}

// HasLevel returns a boolean if a field has been set.
func (o *GetAffiliateInviteeDetailV5RespData) HasLevel() bool {
	if o != nil && !IsNil(o.Level) {
		return true
	}

	return false
}

// SetLevel gets a reference to the given string and assigns it to the Level field.
func (o *GetAffiliateInviteeDetailV5RespData) SetLevel(v string) {
	o.Level = &v
}

// GetRegion returns the Region field value if set, zero value otherwise.
func (o *GetAffiliateInviteeDetailV5RespData) GetRegion() string {
	if o == nil || IsNil(o.Region) {
		var ret string
		return ret
	}
	return *o.Region
}

// GetRegionOk returns a tuple with the Region field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAffiliateInviteeDetailV5RespData) GetRegionOk() (*string, bool) {
	if o == nil || IsNil(o.Region) {
		return nil, false
	}
	return o.Region, true
}

// HasRegion returns a boolean if a field has been set.
func (o *GetAffiliateInviteeDetailV5RespData) HasRegion() bool {
	if o != nil && !IsNil(o.Region) {
		return true
	}

	return false
}

// SetRegion gets a reference to the given string and assigns it to the Region field.
func (o *GetAffiliateInviteeDetailV5RespData) SetRegion(v string) {
	o.Region = &v
}

// GetTotalCommission returns the TotalCommission field value if set, zero value otherwise.
func (o *GetAffiliateInviteeDetailV5RespData) GetTotalCommission() string {
	if o == nil || IsNil(o.TotalCommission) {
		var ret string
		return ret
	}
	return *o.TotalCommission
}

// GetTotalCommissionOk returns a tuple with the TotalCommission field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAffiliateInviteeDetailV5RespData) GetTotalCommissionOk() (*string, bool) {
	if o == nil || IsNil(o.TotalCommission) {
		return nil, false
	}
	return o.TotalCommission, true
}

// HasTotalCommission returns a boolean if a field has been set.
func (o *GetAffiliateInviteeDetailV5RespData) HasTotalCommission() bool {
	if o != nil && !IsNil(o.TotalCommission) {
		return true
	}

	return false
}

// SetTotalCommission gets a reference to the given string and assigns it to the TotalCommission field.
func (o *GetAffiliateInviteeDetailV5RespData) SetTotalCommission(v string) {
	o.TotalCommission = &v
}

// GetVolMonth returns the VolMonth field value if set, zero value otherwise.
func (o *GetAffiliateInviteeDetailV5RespData) GetVolMonth() string {
	if o == nil || IsNil(o.VolMonth) {
		var ret string
		return ret
	}
	return *o.VolMonth
}

// GetVolMonthOk returns a tuple with the VolMonth field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAffiliateInviteeDetailV5RespData) GetVolMonthOk() (*string, bool) {
	if o == nil || IsNil(o.VolMonth) {
		return nil, false
	}
	return o.VolMonth, true
}

// HasVolMonth returns a boolean if a field has been set.
func (o *GetAffiliateInviteeDetailV5RespData) HasVolMonth() bool {
	if o != nil && !IsNil(o.VolMonth) {
		return true
	}

	return false
}

// SetVolMonth gets a reference to the given string and assigns it to the VolMonth field.
func (o *GetAffiliateInviteeDetailV5RespData) SetVolMonth(v string) {
	o.VolMonth = &v
}

func (o GetAffiliateInviteeDetailV5RespData) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetAffiliateInviteeDetailV5RespData) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.AccFee) {
		toSerialize["accFee"] = o.AccFee
	}
	if !IsNil(o.AffiliateCode) {
		toSerialize["affiliateCode"] = o.AffiliateCode
	}
	if !IsNil(o.DepAmt) {
		toSerialize["depAmt"] = o.DepAmt
	}
	if !IsNil(o.FirstTradeTime) {
		toSerialize["firstTradeTime"] = o.FirstTradeTime
	}
	if !IsNil(o.InviteeLevel) {
		toSerialize["inviteeLevel"] = o.InviteeLevel
	}
	if !IsNil(o.InviteeRebateRate) {
		toSerialize["inviteeRebateRate"] = o.InviteeRebateRate
	}
	if !IsNil(o.JoinTime) {
		toSerialize["joinTime"] = o.JoinTime
	}
	if !IsNil(o.KycTime) {
		toSerialize["kycTime"] = o.KycTime
	}
	if !IsNil(o.Level) {
		toSerialize["level"] = o.Level
	}
	if !IsNil(o.Region) {
		toSerialize["region"] = o.Region
	}
	if !IsNil(o.TotalCommission) {
		toSerialize["totalCommission"] = o.TotalCommission
	}
	if !IsNil(o.VolMonth) {
		toSerialize["volMonth"] = o.VolMonth
	}
	return toSerialize, nil
}

type NullableGetAffiliateInviteeDetailV5RespData struct {
	value *GetAffiliateInviteeDetailV5RespData
	isSet bool
}

func (v NullableGetAffiliateInviteeDetailV5RespData) Get() *GetAffiliateInviteeDetailV5RespData {
	return v.value
}

func (v *NullableGetAffiliateInviteeDetailV5RespData) Set(val *GetAffiliateInviteeDetailV5RespData) {
	v.value = val
	v.isSet = true
}

func (v NullableGetAffiliateInviteeDetailV5RespData) IsSet() bool {
	return v.isSet
}

func (v *NullableGetAffiliateInviteeDetailV5RespData) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetAffiliateInviteeDetailV5RespData(val *GetAffiliateInviteeDetailV5RespData) *NullableGetAffiliateInviteeDetailV5RespData {
	return &NullableGetAffiliateInviteeDetailV5RespData{value: val, isSet: true}
}

func (v NullableGetAffiliateInviteeDetailV5RespData) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetAffiliateInviteeDetailV5RespData) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


